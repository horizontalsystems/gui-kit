...
public class ChartView: UIView {
    ...
    public init(configuration: ChartConfiguration, gridIntervalType: GridIntervalType, indicatorDelegate: IChartIndicatorDelegate? = nil) {
        ...
        let pixelsMargin = configuration.showLimitValues ? (configuration.limitTextFont.lineHeight + configuration.limitTextInMargin + configuration.limitTextOutMargin) : 0
    ...
    private func updateInsets() {
        let bottomTextPadding: CGFloat = configuration.showGrid ? ceil(configuration.gridTextFont.lineHeight + configuration.limitTextInMargin) : .zero
