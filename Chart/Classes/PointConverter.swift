...
class PointConverter: IPointConverter {
    ...
    func convert(timestamp: TimeInterval, value: Decimal, viewBounds: CGRect, chartFrame: ChartFrame, retinaShift: Bool) -> CGPoint {
        ...
        let pointOffsetX = CGFloat(timestamp - chartFrame.left) * deltaX
        let pointOffsetY = CGFloat(truncating: (value - chartFrame.bottom) as NSNumber) * deltaY

        let x: CGFloat = ceil(pointOffsetX)
